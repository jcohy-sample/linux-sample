...
usage() {
	...
	echo -e "${BLUE_COLOR}安装 elk 之前，请确保 vm.max_map_count 大小至少为 262144。执行${RES}"
	...
	echo -e "${BLUE_COLOR}安装 mosquitto，请执行  ./docker.sh mosquitto${RES}"
	echo -e "${BLUE_COLOR}安装 emqx，请执行  ./docker.sh emqx${RES}"
  echo -e "${BLUE_COLOR}安装 kafka，请执行  ./docker.sh kafka${RES}"
