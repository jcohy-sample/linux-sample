...
需要注意的是 gitlab 的升级不能跨大版本升级，我们必须先先升级到当前大版本号到最高版本，方可升级到下一个大版本号。可以参考官方给出的版本升级参考 https://gitlab-com.gitlab.io/support/toolbox/upgrade-path[官方推荐升级路线]
...
[NOTE]
====
在最后一步中，注意切换 gitlab 镜像版本。建议在升级过程中，每一次升级成功后都进行一次数据备份，防止下一次升级时失败可以进行回滚。
====
