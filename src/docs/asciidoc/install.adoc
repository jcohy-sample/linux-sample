...
| CentOS Linux release 7.6
| http://mirrors.aliyun.com/centos/7/isos/x86_64/CentOS-7-x86_64-DVD-1908.iso[阿里云镜像下载]
| 无
...
OpenJDK8U-jdk_x64_linux_hotspot_8u292b10

OpenJDK11U-jdk_x64_linux_hotspot_11.0.11_9
| https://software.jcohy.com/jdk/8/jdk-8u221-linux-x64.tar.gz[jdk-8u221]

https://software.jcohy.com/jdk/8/OpenJDK8U-jdk_x64_linux_hotspot_8u292b10.tar.gz[OpenJDK8U-jdk_x64_linux_hotspot_8u292b10]

https://software.jcohy.com/jdk/11/OpenJDK11U-jdk_x64_linux_hotspot_11.0.11_9.tar.gz[OpenJDK11U-jdk_x64_linux_hotspot_11.0.11_9]
| 无

| tomcat
| CentOS Linux release 7.6
| 7.0.94
| https://software.jcohy.com/tomcat/7/apache-tomcat-7.0.94.tar.gz[apache-tomcat-7.0.94]

https://tomcat.apache.org[官方网站]
| 无

| mysql
| CentOS Linux release 7.6
| 8.0.19-1.el7.x86_64
| https://software.jcohy.com/mysql/8/mysql-8.0.19-1.el7.x86_64.rpm-bundle.tar[mysql-8.0.19-1.el7.x86_64]
| 无

| redis
| CentOS Linux release 7.6
| 5.0.5
| https://software.jcohy.com/redis/5/redis-5.0.5.tar.gz[redis-5.0.5.tar.gz]
| 无

| nginx
| CentOS Linux release 7.6
| 1.6.2
| https://software.jcohy.com/nginx/nginx-1.6.2.tar.gz[nginx-1.6.2.tar.gz]
| https://software.jcohy.com/nginx/nginx-1.6.2.build.tar.gz[编译后下载地址]
...
| https://software.jcohy.com/node/node-v10.16.0.tar.gz[node-v10.16.0]
| 无

| yapi
| CentOS Linux release 7.6
| release
| https://github.com/YMFE/yapi/releases[https://github.com/YMFE/yapi/releases]
| 无
...
| CentOS Linux release 7.6
| v2.3.1
| https://software.jcohy.com/harbor/harbor-offline-installer-v2.3.1.tgz[harbor-offline-installer-v2.3.1]
| 无
...
| CentOS Linux release 7.6
| gcc-8.3.0.tar.gz
| http://ftp.gnu.org/gnu/gcc/gcc-8.3.0/gcc-8.3.0.tar.gz[http://ftp.gnu.org/gnu/gcc/gcc-8.3.0/gcc-8.3.0.tar.gz]
| https://software.jcohy.com/gcc/gcc-8.3.0.build.tar.gz[gcc-8.3.0.build]
...
| CentOS Linux release 7.6
| 3.4.5
| https://software.jcohy.com/zookeeper/zookeeper-3.4.5.tar.gz[zookeeper-3.4.5]
| 无
...
[[linux-install-common]]
== 手动安装

[[linux-install-jdk]]
=== JDK

我们提供了 JDK8，OpenJDK8，OpenJDK11 三个安装包，这里只以一个为例，其他的安装过程一样
...
[[linux-install-kafka-usually]]
==== 常规安装

. 用 FileZilla 或其他工具将下载好的安装包上传至 Linux 服务器.放置在 **/opt/software/** 目录下.这里使用的是 **kafka_2.13-3.6.0.tgz**
...
. 配置，修改 `/usr/local/kafka_2.13-3.6.0/config/server.properties` 文件，
+
[source,shell]
----
...
# 配置连接 Zookeeper 集群地址
zookeeper.connect=cluster001:2181,cluster002:2181,cluster003:2181
----
...
. 修改密码加密方式
+
mysql8.0 中默认的加密方式是 `caching_sha2_password` ,修改密码加密方式,改成 `mysql_native_password`,然后修改密码
+
[source,shell]
----
#查看密码加密方式
...
. 修改密码策略
+
mysql8.0 默认安装了密码安全检查插件(`validate_password`),默认密码检查策略要求密码必须包含: 大小写字母、数字和特殊符号,并且长度不能少于8位.否则会提示 ERROR 1819 (HY000): Your password does not satisfy the current policy requirements 错误.
+
[source,shell]
----
#查看密码策略的相关信息
...
如下图所示:
+
image::{oss-images}/linux-install1.jpg[]
...
docker run --name mysql \
    ...
    -v /mydata/mysql/var:/var/lib/mysql \
    -v /mydata/mysql/conf:/etc/mysql/conf.d \
    -v /mydata/mysql/data:/backup \
    -d mysql:5.7
----
...
. https://software.jcohy.com/redis/5/redis-5.0.5.tar.gz[下载] redis 安装包
+
[source,shell]
----
wget https://software.jcohy.com/redis/5/redis-5.0.5.tar.gz
...
https://software.jcohy.com/nginx/nginx-1.6.2.build.tar.gz[下载地址]
...
1.由于 RabbitMQ 依赖 Erlang, 所以需要先安装 Erlang
...
1.在 docker 官网查找 docker 镜像,https://hub.docker.com/

2.拉取镜像,我们选择带有 "mangement" 的版本(包含 web 管理页面)
...
gitlab_rails['ldap_servers'] = YAML.load <<-EOS
   main: # 'main' is the GitLab 'provider ID' of this LDAP server
     ...
     port: 389
     uid: 'cn'
     bind_dn: 'cn=admin,dc=xuanwuai,dc=com'
     password: '123456'
     ...
     base: 'ou=people,dc=xuanwuai,dc=com'
     user_filter: ''
	 attributes:
		username: ['cn']
		...
		last_name:  'sn'
     ## EE only
#     group_base: 'ou=develope,dc=xuanwuai,dc=com'
...
[[linux-install-gitlab-upgrade]]
==== Docker gitlab 升级

由于官方已经开始支持 gitlab 的汉化了，所以开源的社区中文汉化已经不在更新了。最新版的 `twang2218/gitlab-ce-zh` 汉化镜像是基于官方 `gitlab/gitlab-ce:11.1.4-ce.0` 版本进行汉化的。现在我们需要将 gitlab 的版本升级到最新的 16.5.0。

docker 环境部署 gitlab 的升级很简单，直接基于高版本的镜像运行容器即可；
需要注意的是 gitlab 的升级不能跨大版本升级，我们必须先先升级到当前大版本号到最高版本，方可升级到下一个大版本号。可以参考官方给出的版本升级参考 https://docs.gitlab.com/ee/update/index.html#installation-using-docker[官方推荐升级路线]

有关支持的升级路径示例的动态视图，请尝试由 GitLab 支持团队维护的 https://docs.gitlab.com/ee/update/index.html#upgrade-paths[升级路径工具]。

现在我们要从 `11.1.4` 升级到 `16.5.0`, 我们的升级路线为 `11.3.4 -&gt; 11.11.8 -&gt; 12.0.12 -&gt; 12.1.17 -&gt; 12.10.14 -&gt; 13.0.14 -&gt; 13.4.3 -&gt; 13.4.7 -&gt; 13.8.8 -&gt;
13.12.15 -&gt; 14.0.12 -&gt; 14.3.6 -&gt; 14.9.5 -&gt; 14.10.5 -&gt; 15.0.5 -&gt; 15.4.6 -&gt; 15.11.13 -&gt; 16.1.5 -&gt; 16.3.5 -&gt; 16.5.0`

===== 1、数据备份

首先进入 `gitlab/gitlab-ce:11.1.4-ce.0` 镜像，将数据进行备份
...
执行完成后会生成一个 `1613965005_2021_02_22_11.1.4_gitlab_backup.tar` 文件， 完成备份后，需要将文件复制出来留作备份。
...
docker pull gitlab/gitlab-ce:13.4.7-ce.0
...
docker pull gitlab/gitlab-ce:16.3.5-ce.0
...
----
# 进入容器
1、docker exec -i -t gitlab /bin/bash
...
[IMPORTANT]
====
在 14.0.x 之后的版本中进行升级时，请必须确保批量后台迁移完成。不能急于升级。可以在 gitlab 的管理界面 https://localhost/admin/background_migrations 中查看进度。
当队列中所有的 `active` 状态标记为 `finished` 时，才可进行下一步迁移升级。

详情可参考 https://docs.gitlab.com/ee/update/background_migrations.html[批量后台迁移]
====


[NOTE]
====
在最后一步中，注意切换 gitlab 镜像版本。建议每次升级升级成功后，进行数据备份，防止后续升级失败。
====

==== 数据恢复

. 在上一步将数据备份了后，我们重新使用一台机器，重新下载，运行 `twang2218/gitlab-ce-zh` 容器(也可以是 `gitlab/gitlab-ce:11.1.4-ce.0` 容器)。
+
[NOTE]
====
数据恢复的目标版本和源版本必须一致
...
. 将备份的数据上传到 `/opt/gitlab/gitlab-data/backups` 目录
. 进入容器，执行备份
+
[source,shell]
----
...
# 恢复备份
gitlab-rake gitlab:backup:restore BACKUP=1613965005_2021_02_22_11.1.4
----
...
若启动 docker 的时候报错: WARNING IPv4 forwarding is disabled. Networking will not work
...
Harbor 是构建企业级私有 docker 镜像的仓库的开源解决方案，它是 Docker Registry 的更高级封装，它除了提供友好的 Web UI 界面，角色和用户权限管理，用户操作审计等功能外，它还整合了 K8S 的插件(Add-ons)仓 库，即 Helm 通过 chart 方式下载，管理，安装 K8S 插件，而chartmuseum 可以提供存储 chart 数据的仓库【注:helm 就相当于 k8s 的 yum】

另外它还整合了两个开源的安全组件，一个是 Notary，另一个是 Clair，Notary 类似于私有 CA 中心，而 Clair 则是容器安全扫描工具，它通过各大厂商提供的 CVE 漏洞库来获取最新漏洞信息，并扫描用户上传的容器是否存在已知的 漏洞信息，这两个安全功能对于企业级私有仓库来说是非常具有意义的。

[NOTE]
====
注意: Harbor 的所有服务组件都是在 Docker 中部署的,所以官方安装使用 Docker-compose 快速部署,所以我们需要安装 Docker、Docker-compose.由于Harbor 是基于 Docker Registry V2版本,
...
mkdir /opt/software
cd /opt/software
wget https://software.jcohy.com/harbor/harbor-offline-installer-v2.3.1.tgz
tar xvzf harbor-offline-installer-v1.6.0.tgz -C /usr/local
cd /usr/local/harbor
cp harbor.yml.tmpl harbor.yml
----

NOTE::这里跟以前不一样，没有了 `harbor.cfg` 文件，我们需要手动复制 `harbor.yml.tmpl` 在做修改即可
...
注意,此处需要结合第 4 步,登录到控制台后创建 aix 项目.
...
这是原因可能有,一是 Harbor 仓库上 aix/nginx 确实不存在,二是项目未设置公开的时候,该账户未执行 docker login 192.168.11.239 登录操作,三是该账户对 192.168.11.239/aix 项目没有权限,需要在该项目下增加 aix 成员,并选择角色.

(3).如果需要修改 Harbor 的配置文件 `harbor.cfg`,因为 Harbor 是基于 docker-compose 服务编排的,我们可以使用 docker-compose 命令重启 Harbor.不修改配置文件,重启 Harbor 命令: docker-compose `start` | `stop` | `restart`
...
[[linux-install-jenkins-usually]]
==== 本地安装

详情请查看官方文档: https://jenkins.io/zh/doc/book/installing[Jenkins安装]
...
[[linux-install-jenkins-question]]
==== 常见问题
...
+
. 解决方法 1: 进入: `ip:端口/pluginManager/advanced`  修改升级站点中的地址,把 `https:` 改成 `http:` 并且点那个提交按钮.
+
该文件为 jenkins 下载插件的源地址,改地址默认 jenkins 默认为: https://updates.jenkins.io/update-center.json,就是因为 https 的问题,此处我们将其改为 http 即可,之后重启 jenkins 服务即可.
+
image::{oss-images}/jenkins3.jpg[]

. 解决方法2: 修改 `/var/lib/jenkins/updates/default.json`,所以图下的 `google` 改为 `www.baidu.com` 即可,更改完重启服务.

jenkins在下载插件之前会先检查网络连接,其会读取这个文件中的网址.默认是访问谷歌

image::{oss-images}/jenkins4.jpg[]

* connot find the symbol
+
造成此问题的原因是因为此符文所在的文件或者依赖不存在,主要还是因为 jar 包因为出现问题,出现的原因主要有以下两种情况.

. 此项目所依赖的服务版本未更新到最新版本或者未上传发布到私服中央仓库
+
解决方法: 使用 Jenkins 重新构建项目所依赖的微服务,将其生成的 jar 包上传至仓库中.
+
. 如果第一步解决不了,再来进行第二步.可能是因为本地缓存的原因造成的
+
.. 登录到 Jenkins 服务器上
+
.. 进入到目录,找到项目所依赖的微服务的本地缓存,然后删除
+
[source,shell]
----
cd /var/lib/jenkins/.gradle/caches/modules-2/files-2.1
----
+
.. 然后重新构建项目
...
此问题是由于在执行脚本时,用户权限不足所造成的.需要在 Jenkins 的脚本文件中为当前命令添加所需要的权限
...
Jenkins 其他操作请查看 https://www.jenkins.io/doc/[官方网站]
...
#拷贝数据库配置文件,DB_CONIFG 中主要是关于 Berkeley DB 的相关的一些配置
...
https://github.com/jcohy/jcohy-study-sample/blob/master/src/docs/asciidoc/bash/ldap/basedir.ldif[basedir.ldif]

https://github.com/jcohy/jcohy-study-sample/blob/master/src/docs/asciidoc/bash/ldap/domain.ldif[domain.ldif]

https://github.com/jcohy/jcohy-study-sample/blob/master/src/docs/asciidoc/bash/ldap/rootpwd.ldif[rootpwd.ldif]

[[linux-install-ldap-docker]]
==== Docker 安装
...
--name ldap \
--env LDAP_TLS=false \
--env LDAP_ORGANISATION="system" \
--env LDAP_DOMAIN="xuanwuai.com" \
--env LDAP_ADMIN_PASSWORD="ldap@1405" \
--env LDAP_CONFIG_PASSWORD="ldap@1405" \
--restart always \
...
--env PHPLDAPADMIN_LDAP_HOSTS=192.168.11.229 \
--restart always \
...
[[linux-install-confluence-usually]]
==== 常规安装
...
https://software.jcohy.com/confluence/confluence-crack.zip
...
image::{oss-images}/confluence_keygen1.png[]
...
image::{oss-images}/confluence_keygen2.png[]
...
[[linux-install-conflurnce-docker]]
==== docker 安装

- Docker 镜像: https://github.com/cptactionhank[Github链接]
- 破解工具: https://gitee.com/pengzhile/atlassian-agent[Gitee链接]

===== 安装 mysql。请参考 <<linux-install-mysql-docker>> 章节进行安装
...
https://www.cwiki.us/display/CONFLUENCEWIKI/Database+Setup+For+MySQL#DatabaseSetupForMySQL-%E4%BD%BF%E7%94%A8JDBC%E8%BF%9E%E6%8E%A5%EF%BC%88%E9%BB%98%E8%AE%A4%EF%BC%89[修改 mysql 配置]
...
. 下载 https://www.oschina.net/action/GoToLink?url=https%3A%2F%2Fgithub.com%2Fpengzhile%2Fatlassian-agent%2Freleases%2Fdownload%2Fv1.2%2Fatlassian-agent-v1.2.zip[atlassian-agent.jar] 文件，放置在 Dockerfile 同目录下，例如：
+
[source,shell]
----
- Confluence
...
.  打开页面，在要求输入 license 的页面，使用以下命令进行破解 `BXIR-8CP2-3CTL-HW07` 为 id
+
[source,shell]
----
java -jar atlassian-agent.jar -d -m dsf@xuanwuai.com -n BAT -p conf -o http://127.0.0.1 -s BXIR-8CP2-3CTL-HW07
----

[[linux-install-jira]]
=== Jira

[[linux-install-jira-docker]]
==== docker 安装
...
[source,shell]
----
java -jar atlassian-agent.jar -d -m dsf@xuanwuai.com -n BAT -p jira -o http://127.0.0.1 -s BHD4-LKOJ-02MI-SLWO
----

[[linux-install-zookeeper]]
=== zookeeper

[[linux-install-zookeeper-cluster]]
==== 集群安装
机器部署
...
[[linux-install-elasticsearch-docker]]
==== Docker 安装

. 安装 elasticsearch 镜像,使用国内镜像加速
+
[source,shell]
----
...
[[linux-install-script-install-tbl]]
.默认目录
|===
| 变量名 | 目录 | 说明

| BASH_URL | https://software.jcohy.com/bash | 脚本位置

| BASH_CONFIG | https://software.jcohy.com/conf/ | 配置文件位置
